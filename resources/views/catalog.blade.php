        ...
        <div class="sort">
            ...
            <div class="catalog__filter">
                @foreach($categories as $category)
                    <a href="{{ $params->has('sort_by') ? '?sort_by=' . $params['sort_by'] . '&' : ($params->has('sort_by_desc') ? ' ? sort_by_desc=' . $params['sort_by_desc'] . '&' : '?') }}filter={{ $category->id }}"
                       class="catalog__filter-item
                   {{ request()->query('filter') == $category->id ? 'active' : '' }}">{{ $category->product_type }}</a>
                @endforeach
                    ...
                    <div class="w-full max-w-sm bg-white border border-gray-200 rounded-lg shadow dark:bg-gray-800 dark:border-gray-700">
                        ...
                        <div class="px-5 pb-5">
                            ...
                            <div class="flex items-center mt-2.5 mb-5">
                                ...
                                <span class="bg-blue-100 text-blue-800 text-xs font-semibold px-2.5 py-0.5 rounded dark:bg-blue-200 dark:text-blue-800 ms-3">В наличии: {{ $product->qty }}</span>
                            </div>
